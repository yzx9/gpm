...
- `tauri-plugin-biometric-keystore` — a **generic** biometric-gated Keystore seal: stores a caller-supplied secret string in the Android Keystore (AES/GCM, hardware-backed) behind a `BiometricPrompt`. `alias`/`prefs`/`policy`/`prompt` are all caller-supplied — the plugin carries no gpm identifiers or brand strings (the app passes them from `keystore.rs`). gpm uses it for the identity passphrase
- `tauri-plugin-secure-keystore` — a **generic** auth-free-or-biometric-gated Keystore seal, **homomorphic** with `biometric-keystore` (same `store`/`retrieve`/`delete`/`alias_state`/`has_stored` signatures + `KeyPolicy`/`BiometricState` shapes, so the two can merge mechanically); same caller-supplied `alias`/`prefs`/`policy`/`prompt` model. gpm uses it for the at-rest master key (auth-free) and the biometric vault key (App Lock)
