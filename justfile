...
# Run the safe-area plugin's Robolectric unit tests (Android); skipped unless tauri.settings.gradle exists.
[group('test')]
test-plugin:
  @if [ ! -f src-tauri/gen/android/tauri.settings.gradle ]; then \
    ...
    cd src-tauri/gen/android && ./gradlew :tauri-plugin-safe-area:testDebugUnitTest --console=plain; \
  fi
