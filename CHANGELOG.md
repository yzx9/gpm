...
- On **Android**, gpm now warns up front when a store uses an age plugin recipient (such as a YubiKey): you can read its secrets on your phone, but you can't create or edit them there, because the matching age-plugin tool can only run on a desktop. The notice appears during setup, and a save that can't go through now shows as a clear warning instead of a generic error. Such stores were already readable on Android; this just makes the limit obvious before you hit it.
- The **periodic background sync** option (under **AutoSync** in Settings) now appears only on **Android**. It schedules an automatic every-few-hours sync via Android's background scheduler, which desktop doesn't have — so on desktop the toggle did nothing and is now hidden. **AutoSync** itself (sync on every save) is unchanged and works everywhere.
