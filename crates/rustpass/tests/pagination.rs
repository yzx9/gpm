...
// ---------------------------------------------------------------------------
// list / search — ranking + slicing through a real configured Store.
...
#[tokio::test]
async fn list_slices_a_stable_alpha_sorted_window() {
...
async fn list_total_is_independent_of_offset_and_limit() {
    ...
    assert_eq!(store.list(0, 2).await.unwrap().total, 5);
    assert_eq!(store.list(4, 2).await.unwrap().total, 5);
    assert_eq!(store.list(0, 100).await.unwrap().total, 5);
}

#[tokio::test]
async fn list_offset_past_end_is_empty_with_full_total() {
    let (_dir, store) = store_with(vec![("a.age", b"x"), ("b.age", b"x")]).await;

    let p = store.list(10, 5).await.unwrap();
    assert!(p.entries.is_empty());
    assert_eq!(p.total, 2);
}

#[tokio::test]
async fn search_total_is_match_count_not_slice_length() {
    ...
    let p = store.search("aws", 0, 1).await.unwrap();
    assert_eq!(p.entries.len(), 1);
    assert_eq!(p.total, 3);
}

#[tokio::test]
async fn search_ranks_best_match_first_and_pages_stably() {
    let (_dir, store) = store_with(vec![
        ("aws/root.age", b"x"),
        ("drawings/sketch.age", b"x"),
        ("lawyers/contract.age", b"x"),
    ...
    let p = store.search("aws", 0, 10).await.unwrap();
    assert_eq!(p.entries.first().unwrap().name, "aws/root");
    assert_eq!(p.total, 3);

    // Offsetting past the best match drops only it; ranking is stable.
    let p_rest = store.search("aws", 1, 10).await.unwrap();
    assert!(p_rest.entries.iter().all(|e| e.name != "aws/root"));
    assert_eq!(p_rest.total, 3);
}

#[tokio::test]
async fn search_no_match_is_empty_with_zero_total() {
    let (_dir, store) = store_with(vec![("cloud/aws/root.age", b"x")]).await;

    let p = store.search("zzznomatch", 0, 5).await.unwrap();
    assert!(p.entries.is_empty());
    assert_eq!(p.total, 0);
}

#[tokio::test]
async fn search_empty_query_matches_list() {
    // search("") is list: same alpha-sorted full set.
    let (_dir, store) = store_with(vec![("bank.age", b"x"), ("cloud/aws/root.age", b"x")]).await;

    let listed_page = store.list(0, 10).await.unwrap();
    let listed = names(&listed_page.entries);
    let searched_page = store.search("", 0, 10).await.unwrap();
