...
4. **Local config + zeroize-per-decrypt for MVP identity storage, with Android Keystore deferred** — zeroize is ~20 lines of pure Rust with no Android dependency, whereas Keystore adds ~1 day of platform debugging on top of an already ambitious build. Keystore later arrived via the keystore plugin.
