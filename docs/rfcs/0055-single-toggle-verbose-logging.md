...
**What stays.** The Logs viewer, its mtime-ordered tail-truncated read, the rotated file under the app log directory, the clear action, and the frontend error bridge are all unchanged (they shipped with the viewer). Only the level-control mechanism and its UI change.
...
Revises the runtime level-control portion of the in-app diagnostics logging work. The viewer, rotation, plaintext-log threat model (now in SECURITY.md), and frontend error bridge are unchanged. Builds on the recent Debug logger ceiling (no trace-level diagnostics exist in gpm).
