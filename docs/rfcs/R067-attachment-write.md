...
Builds on the shipped attachment read/export (detection, metadata, export). Naturally exercised by the live-binary interop tests (R053), which can drive a real gopass to verify gpm-written attachments round-trip. Independent of, but complementary to, the preview RFC (R068).
